...
- **Steps**:
  ...
  7. `RUN mkdir -p /data /etc/nession && chown www-data:www-data /data`
  8. `WORKDIR /etc/nession`
  9. `EXPOSE 80 443`
  10. `HEALTHCHECK CMD curl -f http://localhost/health || exit 1`
...
# Defaults (envsubst requires variables to be set, no bash-style ${VAR:-default})
LISTEN_PORT="${LISTEN_PORT:-80}"
SERVER_BACKEND="${SERVER_BACKEND:-127.0.0.1:8443}"
export LISTEN_PORT SERVER_BACKEND

# Generate base nginx config from template
envsubst '${LISTEN_PORT} ${SERVER_BACKEND}' \
  < /etc/nginx/templates/default.conf.template \
  > /etc/nginx/conf.d/default.conf

# If TLS certs are provided, append HTTPS server block and add HTTP→HTTPS redirect
if [ -n "$TLS_CERT_PATH" ] && [ -n "$TLS_KEY_PATH" ]; then
  # Insert redirect into the port 80 server block
...
# The server reads config from /etc/nession/config.toml
exec /usr/local/bin/nession-server
```

**Note on server config**: `nession-server` reads its config from `config.toml` in the working directory. The Dockerfile sets `WORKDIR /etc/nession`, and the entrypoint generates `/etc/nession/config.toml` from env vars:

```toml
# Generated by entrypoint.sh
listen_address = "${NESION_LISTEN_ADDRESS:-127.0.0.1:8443}"
auth_token = "${NESION_AUTH_TOKEN}"
db_path = "${NESION_DB_PATH:-/data/server.db}"
tls_cert_path = ""
tls_key_path = ""
heartbeat_timeout_secs = 30
```

The entrypoint generates this file with:
```sh
cat > /etc/nession/config.toml <<EOF
listen_address = "${NESION_LISTEN_ADDRESS:-127.0.0.1:8443}"
auth_token = "${NESION_AUTH_TOKEN}"
db_path = "${NESION_DB_PATH:-/data/server.db}"
tls_cert_path = ""
...
**TLS behavior** (see `entrypoint.sh` above):
- If `TLS_CERT_PATH` + `TLS_KEY_PATH` are both set, the entrypoint appends an HTTPS server block on port 443 and adds an HTTP→HTTPS redirect to the port 80 block
- If not set, only the HTTP block on port 80 is active
