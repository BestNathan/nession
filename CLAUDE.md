...
- **协议门禁**:`just check-protocol`(`scripts/protocol-gate.mjs`,~0.2s),已接入 `pre-commit`(改了 `.rs` 或 `web/src/` 就跑)和 `just check`(CI)。**不认识的消息类型是被忽略而不是被拒绝的** —— 所以一个叫错名字的发送方不会报错、不会被拒,它只是永远等下去;`agent.env.resource` 就这样活了很久(#913)。规则两条,互为对偶:①调用点命名的 wire 必须是某个 runtime 真的会答的(拼错的报 1a,格式对但没人答的报 1b,分开是因为修法不同);②每一个被 advertise 的协议都必须有调用方;③`nession_common::protocol` 这条过渡别名路径不许回来(它 re-export 时不带 family/version,正是 Protocol Unit 模型要回答的问题)—— `crates/nession-common/src/protocol.rs` 已删除,恢复 `pub mod protocol;` 会让所有老 import 重新能编译,编译器看不见这一步,所以由门禁看;④`subscribe` 命名的 wire 必须是协议**声明过**的 —— 推送被 emit 而不被 answer(`agents.changed`/`sessions.changed`/`server.commands.changed` 人人都收得到、没人答),对 listener 问 rule 1 那句「谁答它」问错了问题,所以三条推送在 `web_client_registry.rs` 各自 emitter 旁以 `pub const` 声明,该声明就是 rule 4 判定「声明过没有」的依据(#949)。判定集合不在这里维护,读的是生成树(`just check-codegen` 保证它等于契约)+ kernel 的 `<wire>.response` 规则 + 各 dispatcher 旁边的 `pub const`(通知类 wire,没有路由表能描述它们)。**名字是解析出来的,不要求字面量** —— `msg_types::AGENT_HEARTBEAT` 和 `import { WIRE as ... }` 都比字面量好,要求字面量等于要求更差的写法;报的是「解析不到任何已声明 wire」的名字。两个逃生口,每次运行都会打印出来:行级 `// not-protocol: <理由>`,文件头 `// not-protocol-file: <理由>`(用于主题是传输层、wire 本身就是随便起的文件);两者都覆盖 rule 1 与 rule 4 —— 被检查的就是「这个调用点命名的是什么」。被豁免的**发送方**调用点仍然算作调用方,`subscribe` 除外:listener 不往线上放 wire,算进去等于让 rule 4 自问自答。`just protocol-check-selftest` 把每条规则(含 #913/#949 那两种形态,后者是 listener 那半边)注入 fixture 树,要求门禁以该规则失败 —— 静默失效的门禁会报成功,而成功和「什么都没错」长得一模一样。
