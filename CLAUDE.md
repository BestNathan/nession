...
- **本地编译缓存**（#986）：每个 worktree 有自己的 `target/`，新 worktree 由主 worktree 做 CoW 预热（只带依赖产物）；本地构建统一经 `scripts/rustc-wrapper.sh` 走 sccache。**但 worktree 之间不会复用编译结果** —— sccache 的 key 含绝对路径，Rust 的跨 checkout normalization 上游未完成，所以别指望「新 worktree 零编译」。`just build-cache-status`（配置）与 `just build-cache-verify`（实测，含跨 checkout 命中/未命中）是诊断，不是门禁。详见 `.claude/skills/nession-development/references/local-build-cache.md`。
