...
- **`pre-commit` 只跑快检查**：`scripts/check-dev-workspace.sh commit`（禁止在根目录/`main` 上提交）→ `just quick`（`cargo fmt --check` → `cargo clippy --workspace -D warnings`）+ `just web-lint`（= `just design-check` + `tsc --noEmit`）。不跑测试,不跑覆盖率。
- **Design gate 是唯一入口（#759）**：`just design-check`（`scripts/design-check.sh`）聚合 token/contract/inventory 同步、design 测试套件、规则故障 fixture、以及 web/src 上的 ESLint。**hook 与 CI 都调用它，不各自列 design 检查**——`just web-lint` 依赖它并只多加 `tsc`。改动 `design/**` 或 `web/eslint-plugin-nession/**`（即便没有 `.tsx`）也会跑它：这正是之前漏掉的一类——只改 `design/tokens/*.json` 时两个分支都不匹配，pre-commit 一个 token 检查都没跑。
...
- **CI 触发**：`quality.yml`（PR -> staging:rust-check = `just check` = fmt + lint + check-tmux-socket + coverage,web-check = `just web-lint`（含 design gate）+ `just web-test`）;`staging.yml`（push to staging,纯文档改动经 `paths-ignore` 跳过:完整 build + deploy）;`release.yml`（push to main:release,全部 job 门禁在 `version_changed` 上）。
