...
const DEFAULT_PREFS: AttachPrefs = { mode: 'auto' };

function isAttachMode(v: unknown): v is AttachMode {
  return v === 'auto' || v === 'p2p' || v === 'relay';
}

/** Read last-used attach mode from localStorage, falling back to 'auto'. */
export function loadAttachPrefs(): AttachPrefs {
  try {
    ...
    const parsed = JSON.parse(raw) as Partial<AttachPrefs & { envFiles?: unknown }>;
    return {
...
/** Persist attach mode for next time. Failures are non-fatal. */
